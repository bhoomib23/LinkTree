...
// 1. We keep your exact component logic here
    ...
    return (
        <div className="bg-[#E9C0E9] min-h-screen grid grid-cols-2 pt-15">
            <div className="col1 flex flex-col text-gray-900 justify-center items-center">
                <div className="flex flex-col gap-5 my-8">
                    ...
                    <div className="item">
                        ...
                        <div className="mx-4">
                            <input value={handle || ''} onChange={e => { sethandle(e.target.value) }} className="bg-white my-2 px-4 py-2 focus:outline-pink-400 rounded-4xl" type="text" placeholder='Choose a Handle' />
                        </div>
                    </div>
                    <div className="item">
                        ...
                        {links && links.map((item, index) => {
                            return <div key={index} className="mx-4">
                                <input value={item.linktext || ''} onChange={e => { handlechange(index, item.link, e.target.value) }} className="bg-white my-2 mx-2 px-4 py-2 focus:outline-pink-400 rounded-4xl" type="text" placeholder='Enter link text' />
                                <input value={item.link || ''} onChange={e => { handlechange(index, e.target.value, item.linktext) }} className="bg-white my-2 mx-2 px-4 py-2 focus:outline-pink-400 rounded-4xl" type="text" placeholder='Enter link ' />
                            </div>
                        })}
            ...
            <div className="col2 w-full h-screen bg-[#E9C0E9]">
                <img alt="Promotional Banner" className="h-full object-contain" src="/generate.png" />
            </div>
            <ToastContainer />
        </div>
    )
...
// 2. Wrap it inside Suspense for the main default export
export default function page() {
    return (
        <Suspense fallback={<div className="bg-[#E9C0E9] min-h-screen flex items-center justify-center text-gray-900 font-bold">Loading...</div>}>
            <GeneratePageContent />
        </Suspense>
    )
}
